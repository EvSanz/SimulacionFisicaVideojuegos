...
public:
	...
	void update(double t)
	{
		...
		if (up)
		{
			if (capsula->getPosition().y < 80.0)
				movY = 0.01;
		}

		else
		{
			if (capsula->getPosition().y > 5.0)
				movY = -0.01; 
		}	

		alas->setPosition(alas->getPosition() + Vector3(0.0, movY, 0.0));
		capsula->setPosition(capsula->getPosition() + Vector3(0.0, movY, 0.0));
	}
